...
async function refreshToken(): Promise<any> {
  try {
    ...
    if (!newTokens.refresh_token && oAuth2Client.credentials.refresh_token) {
...
async function youTubeApiPolling() {
  try {
    if (!isMonitoring) {
      ...
      const connected = await connectToYouTubeLivestream(streamInfo.videoId);
    ...
    console.log("CATCH: Failed to reach YouTube API. Trying to refresh token.");
    refreshToken();
  }
}
...
function processYouTubeMessage(item: youtube_v3.Schema$LiveChatMessage): void {
  ...
  if (snippet.type === "textMessageEvent" && snippet.textMessageDetails) {
    unifiedMessage.message.text = snippet.textMessageDetails.messageText || "";
