...
import { bold, Client, Collection, EmbedBuilder, Events, GatewayIntentBits, REST, Routes, SlashCommandBuilder, TextChannel } from 'discord.js'
...
export function DiscordSetup()
{
    ...
    client.once(Events.ClientReady, c => {
        ...
        streamAlertChannel = ( client.channels.cache.get(streamAlertChannelId) as TextChannel )
        allTimeLeaderboardChannel = ( client.channels.cache.get(allTimeLeaderboardChannelId) as TextChannel )
        bimonthlyLeaderboardChannel = ( client.channels.cache.get(bimonthlyLeaderboardChannelId) as TextChannel )
    });
    ...
    allTimeLeaderboardChannel.messages?.fetch().then(messages =>
        {
            ...
            {
                leaderboardMessage += `${i+1} - ${leaderboard[i].Username}: ${leaderboard[i].Score}pts\n`
            }

            leaderboardMessage = bold('All-Time Quiz Leaderboards - Top 50!') + `\n\n` + leaderboardMessage.replace(/(\*|_|`|~|\\)/g, '\\$1')

            if (messages.size > 0)
            {
                messages.last()?.edit(leaderboardMessage)
            }
            else
            {
...
export function PublishBimonthlyLeaderboard(leaderboard: any)
{
    bimonthlyLeaderboardChannel.messages?.fetch().then(messages =>
        {
            ...
            leaderboardMessage = bold('Bi-Monthly Quiz Leaderboards - Top 50!') + `\n\n` + leaderboardMessage.replace(/(\*|_|`|~|\\)/g, '\\$1')
